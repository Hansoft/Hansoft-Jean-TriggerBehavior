...
{
    class DefaultEnumColumnToken : IExpressionItem, IAssignable
    {
        ...
        public DefaultEnumColumnToken(EHPMTaskField taskField, EHPMProjectDefaultColumn column, string property)
        {
        ...
        {
            return new ExpressionValue(ExpressionValueType.STRING, task.GetType().GetProperty(property).GetValue(task).ToString());
        }
        ...
        {
            task.GetType().GetProperty(property).SetValue(task, HansoftEnumValue.FromString(task.ProjectID, column, value.ToString()));
        }
    }
}